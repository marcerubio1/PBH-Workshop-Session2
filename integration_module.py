...
def I_E(k,d,p,a):
    ...
    if isinstance(k,(ndarray)):
        ...
        output =  full(len(k), nan)
                
        for i,j in enumerate(k):
                    
            output[i] = j**(p-2)*J(p,b(j,a,d))
            
        return output
    
    else:
    
        return k**(p-2)*J(p,b(k,a,d))
    ...
    if p == 3:
        ...
        if isinstance(k,(ndarray)):
            ...
            for i,j in enumerate(k):
                ...
                output[i] = sin(j*d)/d - j*Ci(j*d)

            return output

        else:
        
            return sin(k*d)/d - k*Ci(k*d)
        ...
        raise ValueError("Problem with p. Only two possibilities p = 2 or p = 3.")
